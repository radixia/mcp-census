...
| `C1` | Card against runtime | Compare what the card declares against what the handshake answers. Pure comparison of evidence `D1` and `D5` already collected; **no request**. `version` and `protocolVersion` can contradict; `name` is reported as divergence only. **Measured, not scored.** | `MUST NOT contradict the server's actual runtime behavior`, experimental-ext-server-card#23, closed 2026-06-08 |
